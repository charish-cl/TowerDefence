...
public class DragDropBunny : MonoBehaviour
{
    ...
    void Update()
    {
        //if we have money and we can drag a new bunny 钱+拖动状态+点击
            ...
            GameManager.Instance.MoneyAvailable >= Constants.BunnyCost)
        {
                ...
                Physics2D.OverlapPoint(location, 1 << LayerMask.NameToLayer("BunnyGenerator")))
            {
                ...
                isDragging = true;
                //create a temp bunny to drag around  拖动就创建啊
        ...
        //we're stopping dragging  停止拖动，检查落脚点
        else if (Input.GetMouseButtonUp(0) && isDragging)
        {
            ...
            //in order to place it, we must have a background and no other bunnies   确保放置在合适的位置
            if (hits.Where(x=>x.collider.gameObject.tag == "Background").Count() > 0
                && hits.Where(x => x.collider.gameObject.tag == "Path").Count() == 0
                && hits.Where(x=>x.collider.gameObject.tag == "Bunny").Count() == 1)
            {
                //we can leave a bunny here, so decrease money and activate it     可以放置
    ...
    //make background sprite appear as it is  兔子选择框的背景
