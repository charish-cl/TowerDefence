...
public class Bunny : MonoBehaviour
{
    ...
    void Update()
    {
        //if we're in the last round and we've killed all enemies, do nothing  待机状态
        if (GameManager.Instance.FinalRoundFinished &&
            GameManager.Instance.Enemies.Where(x => x != null).Count() == 0)
            State = BunnyState.Inactive;

        //searching for an enemy 发现敌人
        ...
        else if (State == BunnyState.Targeting) //瞄准状态
        {
            ...
            else //enemy has left our shooting range, so look for another one 敌人离开视线，很严谨呀
            {
